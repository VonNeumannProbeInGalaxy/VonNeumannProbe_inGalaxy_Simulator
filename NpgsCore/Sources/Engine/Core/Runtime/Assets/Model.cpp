...
void FModel::InitModel(const std::string& Filename)
{
    ...
    const aiScene* Scene = Loader.ReadFile(FilePath, Flags);
    if (!Scene || Scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !Scene->mRootNode)
    {
        NpgsCoreError("Fatal error: Assimp: {}", Loader.GetErrorString());
        std::system("pause");
...
std::vector<FMesh::FTextureData>
FModel::LoadMaterialTextures(const aiMaterial* Material, const aiTextureType& TextureType, const std::string& TypeName)
{
    std::vector<FMesh::FTextureData> Textures;
