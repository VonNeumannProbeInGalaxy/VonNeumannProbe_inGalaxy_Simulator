...
public:
    ...
    std::vector<std::pair<OrbitalElements, std::vector<AstroObject::AstroObject*>>>& OrbitData();

private:
    BaryCenter _SystemBary;
    std::vector<AstroObject::Star>   _Stars;
    std::vector<AstroObject::Planet> _Planets;
    std::vector<std::pair<OrbitalElements, std::vector<AstroObject::AstroObject*>>> _Orbits;
};
