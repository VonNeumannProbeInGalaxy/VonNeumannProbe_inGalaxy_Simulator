...
FApplication::FApplication(const vk::Extent2D& WindowSize, const std::string& WindowTitle,
	...
	:
	_VulkanBase(FVulkanBase::GetInstance()),
...
void FApplication::ExecuteMainRender()
{
	...
	std::vector<FVulkanFence> InFlightFences(FramesInFlightCount);
	std::vector<FVulkanSemaphore> Semaphores_ImageAvailable(FramesInFlightCount);
	std::vector<FVulkanSemaphore> Semaphores_RenderFinished(FramesInFlightCount);
	std::vector<FVulkanCommandBuffer> CommandBuffers(FramesInFlightCount);

	vk::FenceCreateFlags FenceCreateFlags{ vk::FenceCreateFlagBits::eSignaled };
	for (std::size_t i = 0; i != FramesInFlightCount; ++i)
	{
		InFlightFences[i] = FVulkanFence(_VulkanCore->GetDevice(), FenceCreateFlags);
		Semaphores_ImageAvailable[i] = FVulkanSemaphore(_VulkanCore->GetDevice());
		Semaphores_RenderFinished[i] = FVulkanSemaphore(_VulkanCore->GetDevice());
	}

	FVulkanCommandPool CommandPool(_VulkanCore->GetDevice(), _VulkanCore->GetGraphicsQueueFamilyIndex(),
...
void FApplication::CreateScreenRender()
{
	...
	_Renderer.RenderPass = FVulkanRenderPass(_VulkanCore->GetDevice(), RenderPassCreateInfo);
...
void FApplication::CreatePipelineLayout()
{
	...
	_VulkanPipelineLayout = FVulkanPipelineLayout(_VulkanCore->GetDevice(), PipelineLayoutCreateInfo);
}

void FApplication::CreatePipeline()
{
	static FVulkanShaderModule VertShaderModule(_VulkanCore->GetDevice(), "Sources/Shaders/Triangle.vert.spv");
	static FVulkanShaderModule FragShaderModule(_VulkanCore->GetDevice(), "Sources/Shaders/Triangle.frag.spv");
...
#include "Vertices.inc"
	...
	auto Create = [this, VulkanCore = _VulkanCore]() -> void
	{
		...
		FVulkanPipeline NewPipeline(_VulkanCore->GetDevice(), Pack);
		_VulkanCore->WaitIdle();
		if (_VulkanPipeline)
		{
			FVulkanPipeline OldPipeline = std::move(_VulkanPipeline);
	...
	auto Destroy = [VulkanCore = _VulkanCore, VulkanPipeline = &_VulkanPipeline]() -> void
	{
		...
		{
			VulkanCore->WaitIdle();
			FVulkanPipeline OldPipeline = std::move(*VulkanPipeline);
		}
	};
